const { app, Menu, BrowserWindow, protocol, ipcMain, dialog, Electron } = require("electron");
const fs = require("fs");
...
app.on("ready", () => {
	protocol.registerFileProtocol("avc", (request, callback) => callback(decodeURI(request.url.replace(/^avc:\//, ""))));
});
...
function versionResetCheck() {
	...
	if (lastVer != version) {
		if (config.get("customBG") === "https://cdn.discordapp.com/attachments/983598732505411595/1181099378149175358/image_13_1.png'") {
			config.delete("customBG");
		}
		if (config.get("customCrosshairImage") === "https://cdn.discordapp.com/attachments/616206938048561152/996062796892614716/KovaaK-Crosshair_1.png") {
			config.delete("customCrosshairImage");
		}
		if (config.get("customLogo") === "https://cdn.discordapp.com/attachments/983598732505411595/1181131421830623312/image_3-min.png") {
...
function createSplashWindow() {
	...
	const update = async () => {
		...
		autoUpdater.on("checking-for-update", () => {
			splashWindow.webContents.send("status", "Checking for updates from " + autoUpdater.getFeedURL() + "...");
			updateCheck = setTimeout(() => {
				splashWindow.webContents.send("status", "Error while checking for update!");
				setTimeout(() => {
					createMainWindow();
				}, 1000);
			}, 15000);
		...
		autoUpdater.on("update-not-available", () => {
			if (updateCheck) clearTimeout(updateCheck);
			splashWindow.webContents.send("status", "Found latest version!");
			setTimeout(() => {
				createMainWindow();
			}, 1000);
		});
		autoUpdater.on("error", (e) => {
			if (updateCheck) clearTimeout(updateCheck);
			splashWindow.webContents.send("status", "Error while checking for updates from \"" + autoUpdater.getFeedURL() + "\": " + e.message);
			setTimeout(() => {
				createMainWindow();
			}, 1000);
		});
		autoUpdater.on("download-progress", (i) => {
			if (updateCheck) clearTimeout(updateCheck);
			splashWindow.webContents.send("status", "Downloading update...");
		});
...
function createMainWindow() {
	...
	mainWindow.webContents.session.webRequest.onBeforeRequest((details, callback) => {
		if (config.get("resourceSwapperEnable") && files.includes(json[details.url])) {
			callback({
				redirectURL: "avc://" + path.join(app.getPath("documents"), "/AVC-Swapper", json[details.url]),
			});
...
const rejectJson = () => {
	const filePath = swapper().includes("rejectConfig.json") ? path.join(app.getPath("documents"), "/AVC-Swapper", "rejectConfig.json") : path.join(__dirname, "js/rejectConfig.json");
	try {
...
const swapperJson = () => {
	const filePath = swapper().includes("swapperConfig.json") ? path.join(app.getPath("documents"), "/AVC-Swapper", "swapperConfig.json") : path.join(__dirname, "js/swapperConfig.json");
	try {
		let data = fs.readFileSync(filePath, "utf8");
		const jsonContent = JSON.parse(data);
		return jsonContent;
...
app.whenReady().then(() => {
	...
	const dateString = today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate() + "-" + today.getHours() + "-" + today.getMinutes() + "-" + today.getSeconds();
